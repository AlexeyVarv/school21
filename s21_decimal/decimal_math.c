...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  int operation = S21_ARITHMETIC_OK;
...
void print_value_bits_struct_decimal(s21_decimal value) {
  printf("Bits[0]: %d bits[1]: %d bits[2]: %d Scale: %d Sign: %d\n",
...
int setBitToZero(unsigned int num, int position) {
  unsigned int mask = ~(
      1 << position); // Создаем маску с установленным битом в позиции position
  return num &
         mask; // Устанавливаем 0 бит в позиции position и возвращаем результат
}
